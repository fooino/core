...
describe('NormalizesInputs trait', function () {
    ...
    it('normalizes and converts blank to null by default', function () {
        NormalizesInputsTestFormRequest::$testRules = ['title' => 'required', 'bio' => 'nullable'];

        $request = resolveRequest(
            request: NormalizesInputsTestFormRequest::class,
            data: ['title' => 'عليك سلام', 'bio' => ''],
        );
    ...
    it('applies a single pipe after nullIfBlank', function () {
        ...
        NormalizesInputsTestFormRequest::$testConfigs = ['slug' => ['pipe' => fn($v) => str_replace(' ', '-', strtolower($v))]];
    ...
    it('chains multiple pipes in sequence', function () {
        ...
        NormalizesInputsTestFormRequest::$testConfigs = ['phone' => ['pipe' => [
            fn($v) => str_replace(',', '', $v),
    ...
    it('runs pipes after nullIfBlank so pipes can opt to keep null', function () {
        NormalizesInputsTestFormRequest::$testRules = ['slug' => 'nullable'];
        NormalizesInputsTestFormRequest::$testConfigs = ['slug' => ['pipe' => [
    ...
    it('uses nullOnZero with default fallback', function () {
        NormalizesInputsTestFormRequest::$testRules = ['count' => 'nullable'];
        NormalizesInputsTestFormRequest::$testConfigs = ['count' => ['nullOnZero' => true, 'default' => 0]];
    ...
    it('normalizes Arabic and Persian digits', function () {
        NormalizesInputsTestFormRequest::$testRules = ['phone' => 'required'];

        $request = resolveRequest(
            request: NormalizesInputsTestFormRequest::class,
            data: ['phone' => '۰۱۲۳۴۵۶۷۸۹'],
        );
    ...
    it('replaces Arabic letters ي and ك with Persian ی and ک', function () {
        NormalizesInputsTestFormRequest::$testRules = ['name' => 'required'];
    ...
    it('strips non-allowed HTML tags', function () {
        NormalizesInputsTestFormRequest::$testRules = ['body' => 'required'];

        $request = resolveRequest(
            request: NormalizesInputsTestFormRequest::class,
            data: ['body' => '<b>safe</b><script>alert(1)</script>'],
        );
    ...
    it('passes the request instance to pipes', function () {
        NormalizesInputsTestFormRequest::$testRules = ['slug' => 'nullable', 'title' => 'required'];
        NormalizesInputsTestFormRequest::$testConfigs = ['slug' => [
    ...
    it('preserves pipe results when value is not blank', function () {
        NormalizesInputsTestFormRequest::$testRules = ['phone' => 'nullable'];
        NormalizesInputsTestFormRequest::$testConfigs = ['phone' => ['pipe' => fn($v) => str_replace('-', '', $v)]];
